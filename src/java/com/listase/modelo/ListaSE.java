...
public class ListaSE implements Serializable {
    ...
    public float promediasEdades() {
        ...
        if (cabeza != null) {
            ...
            return sumaEdades / (float) cont;
        }
        return 0;

    }

    public void invertrLista() {
        if (cabeza != null) {
            //crear una lista temp, cabeza vacia
            ListaSE listaTemporal = new ListaSE();
            //llamoa ayudante
            Nodo tem = cabeza;
            //recorrer la lista 
            while (tem != null) {
                listaTemporal.adicionarNodoAlInicio(tem.getDato());
            }
